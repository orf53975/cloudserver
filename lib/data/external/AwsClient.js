...
const { logHelper, removeQuotes, trimXMetaPrefix, modifyUserAgent } = require('./utils');
...
modifiedUserAgent = modifyUserAgent('APN/1.0 Scality/1.0 Zenko/1.0');
...
class AwsClient {
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        if (keyContext.isDeleteMarker) {
			return this._client.deleteObject(params)
				.on('build', modifiedUserAgent).send(putCb);
        }
        ...
        if (!stream) {
			return this._client.putObject(uploadParams)
				.on('build', modifiedUserAgent).send(putCb);
        }

		uploadParams.Body = stream;
		// TODO Investigate managed upload
    ...
    head(objectGetInfo, reqUids, callback) {
        ...
        }).on('build', modifiedUserAgent).send(err => {
            if (err) {
    ...
    get(objectGetInfo, range, reqUids, callback) {
		...
		request.on('build', modifiedUserAgent);
