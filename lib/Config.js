...
function hyperdriveClientLocationConstraintAssert(location, locationObj) {
    /* Hack - overwrite and set some default config
     * for the mid-October demo */

    console.log('location', location);
    console.log('locationObj before', locationObj);
    
    const conf = locationObj.details.connector.hdclient;
    conf.policy = {
        minSplitSize: 512000,
        locations: Object.keys(conf.uuidmapping),
    };
    if (conf.codingParts > 0) {
        conf.code = 'RS';
    } else {
        conf.code = 'CP';
    }
    conf.requestTimeoutMs = 10000;
    conf.errorAgent = { kafkaBrokers: 'unused' };

    /* Error agent should be overwritten - Kafka setup out of demo scope */
    hdclient.hdclient.HyperdriveClient.prototype.setupErrorAgent = function mockedSetup() {
        this.errorAgent = {
    ...
    hdclient.hdclient.HyperdriveClient.prototype.destroyErrorAgent = function mockedDestroy() {
        this.errorAgent = null;
    };

    console.log('conf', conf);
    console.log('locationObj', locationObj.details.connector.hdclient);

    const { configIsValid, configError } = hdclient.config.validate(locationObj.details.connector.hdclient);
    if (!configIsValid) {
        throw configError;
    }
}
